...
function runsDir(): string {
  return process.env.DATA_DIR ?? path.join(process.cwd(), "data", "runs");
}
...
export async function loadRun(id: string): Promise<Run | null> {
  ...
  try {
    return JSON.parse(await readFile(fileFor(id), "utf8")) as Run;
  } catch (err) {
...
async function readSummary(file: string): Promise<RunSummary | null> {
  try {
    const run = JSON.parse(await readFile(path.join(runsDir(), file), "utf8")) as Partial<Run>;
