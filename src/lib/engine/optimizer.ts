...
import { CONFLICTS, DISTRICTS, INDICATORS, MEASURES, SCORE_WEIGHTS, SYNERGIES, WEIGHTS } from "@/lib/data";
import { DEFAULT_DATASET, isDefaultDataset, type Dataset } from "@/lib/dataset";
...
// ---------------------------------------------------------------------------
// Precomputed tables for one dataset: measures, districts and indicators as indices, effects as flat
// numbers. Built once per dataset; closures keep the hot loops on local constants.
// ---------------------------------------------------------------------------
...
interface FastModel {
  isValidFast(decisions: readonly Decision[]): boolean;
  fastScoreOf(decisions: readonly Decision[]): number;
  enumerateAll(): Optimum;
}

function makeFastModel(ds: Dataset): FastModel {
  const { measures: MEASURES, districts: DISTRICTS } = ds;
  ...
  const W = INDICATORS.map((k) => ds.weights[k]);
  const POP = DISTRICTS.map((d) => d.population);

  const PAIR = (list: { pair: [string, string] }[]) =>
    list.map((x) => [M_INDEX.get(x.pair[0] as Decision["measureId"]) ?? -1, M_INDEX.get(x.pair[1] as Decision["measureId"]) ?? -1]);
  const CONFLICT_PAIRS = PAIR(ds.conflicts);
  const CONFLICT_SAME = ds.conflicts.map((c) => c.sameDistrictOnly);
  const SYNERGY_PAIRS = PAIR(ds.synergies);
  const SYNERGY_IND = ds.synergies.map((s) => INDICATORS.indexOf(s.indicator));
  const SYNERGY_BONUS = ds.synergies.map((s) => s.bonus);
  ...
  /** Same-district conflicts (M4/M7, M5/M13 in the case data). */
  ...
  function isValidFast(decisions: readonly Decision[]): boolean {
  ...
  function fastScoreOf(decisions: readonly Decision[]): number {
  ...
  return { isValidFast, fastScoreOf, enumerateAll };
}

const DEFAULT_MODEL = makeFastModel(DEFAULT_DATASET);

/** Inline equivalent of `validate(scenario).ok` on the case data, checked against it in optimizer.test.ts. */
export function isValidFast(decisions: readonly Decision[]): boolean {
  return DEFAULT_MODEL.isValidFast(decisions);
}
...
export function fastScoreOf(decisions: readonly Decision[]): number {
  return DEFAULT_MODEL.fastScoreOf(decisions);
}

export function enumerateAll(ds: Dataset = DEFAULT_DATASET): Optimum {
  return ds === DEFAULT_DATASET ? DEFAULT_MODEL.enumerateAll() : makeFastModel(ds).enumerateAll();
}
...
function datasetHash(ds: Dataset): string {
  const { districts, measures, weights, synergies, conflicts } = ds;
  return createHash("sha256").update(JSON.stringify({ districts, measures, weights, synergies, conflicts })).digest("hex");
}

export function getSandboxOptimum(ds: Dataset): Optimum {
  const key = datasetHash(ds);
  const hit = sandboxOptimum.get(key);
  if (hit) {
    sandboxOptimum.delete(key);
    sandboxOptimum.set(key, hit);
    return hit;
  }
  const computed = enumerateAll(ds);
  sandboxOptimum.set(key, computed);
  if (sandboxOptimum.size > SANDBOX_CACHE_LIMIT) sandboxOptimum.delete(sandboxOptimum.keys().next().value!);
  return computed;
}
...
function buildCandidates(ds: Dataset): Decision[] {
  ...
  for (const m of ds.measures) {
    if (m.scope === "city") {
      ...
      for (const d of ds.districts) list.push({ measureId: m.id, districtId: d.id });
    }
  }
  return list;
}
...
function describeChange(before: Decision, after: Decision, ds: Dataset): string {
  ...
  if (after.measureId === before.measureId) {
    ...
    const m = measureIndex(ds).get(after.measureId)!;
    afterLabel =
      m.scope === "district"
...
function neighbourImprovements(decisions: Decision[], userScore: number, ds: Dataset): Improvement[] {
  ...
  for (let i = 0; i < decisions.length; i++) {
    for (const cand of candidates) {
      ...
      const score = scoreOf(next, [], ds);
      if (score <= userScore + EPS) continue;
      const key = decisionKey(next);
      if (seen.has(key)) continue;
      seen.add(key);
      found.push({ decisions: next, score, change: describeChange(decisions[i], cand, ds) });
    }
  }
...
// Sandbox datasets get the same full enumeration from tables built for that dataset, cached in memory only.
export async function optimize(scenario: Scenario, ds: Dataset = DEFAULT_DATASET): Promise<OptimizerResult> {
  const decisions = scenario.decisions ?? [];
  const sandbox = !isDefaultDataset(ds);
  const userScore = scoreOf(decisions, [], ds);
  const optimum = sandbox ? getSandboxOptimum(ds) : await getOptimum();

  return {
    bestScore: optimum.bestScore,
    bestScenario: optimum.bestScenario,
    percentile: percentileOf(optimum.scores, userScore),
    improvements: neighbourImprovements(decisions, userScore, ds),
  };
}
