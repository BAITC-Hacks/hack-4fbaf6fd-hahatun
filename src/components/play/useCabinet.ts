...
export function useCabinet(initial: Decision[]) {
  const router = useRouter();
  const [decisions, setDecisions] = useState(initial);
  const [submitting, setSubmitting] = useState(false);
  const summary = useMemo(() => summarizeSet(decisions), [decisions]);

  function submit() {
    if (!summary.validation.ok || submitting) return;
    setSubmitting(true);
    savePendingRun({ teamName: getTeamName() || FALLBACK_TEAM, scenario: { decisions } });
