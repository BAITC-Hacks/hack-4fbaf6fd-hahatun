...
**Оптимизатор (реализован как A5-lite 23.09 15:00):** только соседи по одной замене (≤ 270 наборов), перцентиль и «лучший Score» считаются среди соседей, факты об этом так и сформулированы. Исходный план ниже сохранён как цель развития. Полный перебор. 14 мер → C(14,5)=2002 комбинации ID, фильтр по лимиту 2 на направление и конфликтам, затем перебор районов для district-мер. Ожидается < 3 млн наборов, TS считает секунды. Результат кэшируется в `data/optimum.json` при первом запуске. Перцентиль = доля валидных наборов со Score ≤ пользовательского. Улучшения = лучшие 3 соседа (замена одной меры или её района), каждый валиден.
...
8. `persist`: запись `Run` в JSON `data/runs/<id>.json`, событие `done` с `runId`.
...
- 2026-09-23 15:40: модели `gpt-5.4-mini` (эксперты) и `gpt-5.5` (судьи); `reasoningEffort` у них `none|low|…`, `minimal` не принимается. Схемы LLM только с `nullable`, без `optional` (strict JSON schema OpenAI). Ревизор и арбитр при сбое LLM уходят в эвристику/детерминированную резолюцию, тексты арбитра проходят проверку чисел как C1.

## 12a. Что добавлено после MVP (15:00–15:30)

- Оптимизатор: полный перебор 694 395 валидных наборов за ~0.4 с, кэш `data/optimum.json` с версией-хэшем датасета. Перцентиль среди всех валидных, глобальный оптимум 57.24 (M2, M3 Нура, M8 Нура, M9 Нура, M14).
- События (A14): `GET /api/runs/[id]/event`. Событие выбирается детерминированно из id прогона, шок бьёт по показателю района поверх исходного сценария, `suggestion` — лучшая замена одной меры под шоком. Перезапуск консилиума под шоком не делается, кнопка ведёт в кабинет с предложенным набором.
- Экспорт презентации (A15): `GET /api/runs/[id]/pitch`, markdown только из полей `Run`.
